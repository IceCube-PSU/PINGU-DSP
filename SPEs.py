        ...
        if len(splits) == 6:
            ...
            # the area and peak pulseheight under the putative pulse
                 ...
                 if len(xvals) > 200: # skip cases with too few digitizations
                    ...
                    central_bin = 155      # eyeballing pulses, they mostly start at 640ns or bin 160
                    area_offset_left  = 5  # number of bins to left of central bin used to find area
                    area_offset_right = 15 # number of bins to right of central bin used to find area
                    baseline_offset = 100  # use bins between baseline_offset and area_offset_left
                                           #    to left of central bin to calculate running baseline
                
                    baseline_yvals = yvals[central_bin-baseline_offset:central_bin-area_offset_left]
                    baseline = np.average(baseline_yvals)
                    baseline_std = np.std(baseline_yvals)

                    yvals_bc = [] # baseline-corrected ADC counts
                    for j in range(central_bin-area_offset_left,central_bin+area_offset_right):
                        yval_baseline_corrected = float(yvals[j]) - float(baseline) # remove baseline
                        yvals_bc.append(yval_baseline_corrected) # baseline-corrected adc counts
                    area = np.sum(yvals_bc)
                    peak = np.amax(yvals_bc)
                    Histograms[1].Fill(area)
                    Histograms[2].Fill(peak)                       
...
def InitializeHistograms(infile,oscilloscope,DDC2):
    ...
    if DDC2:
        ...
        PeaksHistogram = ROOT.TH1D('PeaksHistogram',
                                       'Peak pulse in signal window',
                                        50,
                                         0.0,
                                         50.0)
        AreasHistogram.GetXaxis().SetTitle('Counts')
        AreasHistogram.GetYaxis().SetTitle('Entries')
        AreasHistogram.SetTitle('Peak in pulse window (baseline subtracted) %s %s %s %s' % 
                                (Titlesplits[-4],Titlesplits[-3],Titlesplits[-2],Titlesplits[-1]))


        Histograms.append(BaselinesHistogram)
        Histograms.append(AreasHistogram)
        Histograms.append(PeaksHistogram)
...
def SaveHistograms(Histograms,outdir):
    ...
    Histograms[2].Draw()
    c2.SaveAs(outdir+"/Peaks.png")
    c2.Close()

    RootFile = ROOT.TFile("SPEs.root","recreate")
    Histograms[1].Write()
    Histograms[2].Write()
#    RootFile.Write()
    RootFile.Close()
